  ...
  return (
    <div className="px-4 py-6">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-h1 text-brand-black">Scan Pickup QR</h1>
      </div>
      ...
      {!result && (
        <form onSubmit={handleManualSubmit} className="mt-5 flex gap-2">
          <input
            ...
            className="flex-1 rounded-xl border border-gray-200 px-3 py-2.5 text-input focus:border-brand-red focus:outline-none focus:ring-1 focus:ring-brand-red"
          />
          <button
            type="submit"
            disabled={verifying}
            className="rounded-full bg-brand-black px-5 py-2.5 text-btn-lg text-white disabled:opacity-60"
          >
            Verify
          </button>
        </form>
      ...
      {verifying && <p className="mt-4 text-center text-body-sm text-gray-500">Verifying…</p>}
      ...
      {error && (
        <div className="mt-4">
          ...
          <button
            ...
            className="mt-3 w-full rounded-full border border-gray-200 py-2.5 text-btn-lg text-brand-black"
          >
            Scan again
          </button>
        </div>
      ...
      {result && (
        <div className="mt-5 rounded-2xl border border-gray-100 p-4">
          <div className="flex items-center justify-between">
            <p className="text-h5 text-brand-black">Order #{result.id}</p>
          ...
          <p className="mt-2 text-body-sm font-semibold text-green-600">Order marked as collected.</p>

          <button
            onClick={resumeScanning}
            className="mt-4 w-full rounded-full bg-brand-red py-3 text-btn-lg text-white"
          >
            Scan next order
          </button>
        </div>
      ...
      {!scanning && !result && !error && !verifying && (
        <p className="mt-4 text-center text-body-sm text-gray-400">Camera paused</p>
      )}
    </div>
  );
}
