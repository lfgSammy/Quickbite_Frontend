  ...
  return (
    <li className="rounded-2xl border border-gray-100 p-4">
      <div className="flex items-center justify-between">
        <p className="text-h5 text-brand-black">Order #{order.id}</p>
      ...
      <ul className="mt-2 text-body-sm text-gray-600">
      ...
      <p className="mt-2 text-body font-semibold text-brand-black">{formatNaira(order.total_amount)}</p>
        ...
        <div className="mt-3 flex gap-2">
          {order.status === 'paid' && (
            <button
              ...
              className="flex-1 rounded-full bg-brand-black py-2 text-btn-lg text-white disabled:opacity-60"
            >
              Start preparing
            </button>
          )}
          {order.status === 'preparing' && (
            <button
              ...
              className="flex-1 rounded-full bg-brand-red py-2 text-btn-lg text-white disabled:opacity-60"
            >
              Mark ready
            </button>
          )}
          <button
            ...
            className="rounded-full border border-gray-200 px-4 py-2 text-btn-lg text-gray-500 disabled:opacity-60"
          >
            Cancel
          </button>
        </div>
    ...
    <div className="px-4 py-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <BackButton />
          <h1 className="text-h1 text-brand-black">Kitchen Queue</h1>
        </div>
        <Link
          to="/kitchen/scan"
          className="flex items-center gap-1.5 rounded-full bg-brand-black px-3 py-2 text-btn-sm text-white"
        >
      ...
      {visibleOrders.length === 0 ? (
        <p className="py-16 text-center text-body text-gray-500">No orders here.</p>
      ) : (
